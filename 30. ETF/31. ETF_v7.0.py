# -*- coding: utf-8 -*-
"""
ETF 시세·PDF 수집과 분석·대시보드 (31. ETF_v7.0)

① ETF 시세 수집 (krx_etf_ohlcv, KRX OTP CSV MDCSTAT04301)
② ETF PDF 수집 (krx_etf_pdf, getJsonData finder + MDCSTAT05001)
③ 모멘텀·수익률 대시보드 (etf_dashboard.html / etf_code_name_returns.html)
④ 구성비중 대시보드 (results/etf_pdf_weight_dashboard_{일자}.html, chainΔ 포함)

단계별 on/off: RUN_COLLECT_OHLCV / RUN_COLLECT_PDF / RUN_DASH_MOMENTUM / RUN_DASH_PDF
"""
...
# ---------------------------------------------------------------------------
# 실행 단계 on/off (Spyder 대화식 부분 실행용)
# ---------------------------------------------------------------------------
RUN_COLLECT_OHLCV = True   # ① ETF 시세 수집
RUN_COLLECT_PDF = True     # ② ETF PDF 수집
RUN_DASH_MOMENTUM = True   # ③ 모멘텀·수익률 대시보드
RUN_DASH_PDF = True        # ④ 구성비중 대시보드 (chainΔ)
...
# ============================================================================
# ETF PDF 수집 + 구성비중 대시보드 (구 32. ETF_PDF_v2.0 흡수)
# ============================================================================
...
# TRD_DATES는 collect_etf_pdf()에서 resolve_trd_dates()로 채움
# 저장: <이 스크립트>/results/krx_pdf_result_{수집일}.csv
def _etf_script_dir() -> str:
    try:
        return os.path.dirname(os.path.abspath(__file__))
    except NameError:
        return os.getcwd()

_ETF_SCRIPT_DIR = _etf_script_dir()
PDF_RESULTS_DIR = os.path.join(_ETF_SCRIPT_DIR, "results")
...
# 대시보드(구성비중 스냅샷) 설정
# ─────────────────────────────────────────────────────────────
PDF_WEIGHT_COL = "시가총액기준 구성비중"
PDF_OPEN_BROWSER = True

PDF_DASH_START_DATE = None  # date | str | None — 대시보드 조회 시작일
PDF_DASH_END_DATE = None
PDF_DASH_ETF_FILTER = None  # list[str] | None — None이면 ISU_CODES 전체(그룹 순)
...
PDF_TOP_N = 20
PDF_SUMMARY_TOP_N = 20
PDF_GRAD_MAX_PP = 5.0
EXCLUDE_FROM_WEIGHT_RANK = {"005930", "000660"}
EXCLUDE_NAME_KEYWORDS = ("삼성전자", "하이닉스")


# PDF용 getJsonData 엔드포인트 (OHLCV OTP와 별도; 로그인은 krx_login 공유)
PDF_JSON_URL = f"{_KRX_BASE}/comm/bldAttendant/getJsonData.cmd"
PDF_JSON_HEADERS = {
    "User-Agent": _LOGIN_UA,
    "Referer": f"{_KRX_BASE}/contents/MDC/MDI/outerLoader/index.cmd",
...
# ─────────────────────────────────────────────────────────────
def build_isin_map(session: rq.Session, market: str = "ALL") -> dict:
    """finder로 전체 ETF/ETN/ELW 목록을 받아 {단축코드: ISIN} 딕셔너리 생성."""
    params = {"bld": "dbms/comm/finder/finder_secuprodisu",
              "mktsel": market, "searchText": ""}
    rows = session.post(PDF_JSON_URL, data=params, headers=PDF_JSON_HEADERS, timeout=15).json()
...
def fetch_pdf(session: rq.Session, isin: str, date: str) -> pd.DataFrame:
    params = {"bld": "dbms/MDC/STAT/standard/MDCSTAT05001",
              "trdDd": date, "isuCd": isin}
    out = session.post(
        PDF_JSON_URL, data=params, headers=PDF_JSON_HEADERS, timeout=PDF_HTTP_TIMEOUT
    ).json()
    df = pd.DataFrame(out.get("output", []))
    if df.empty:
        return df
    # 누락 컬럼은 None으로 채워 CSV/DB 스키마 유지
    missing = [src for src in PDF_JSON_COLS if src not in df.columns]
    if missing:
        print(f'  ⚠️ PDF 응답 누락 키 {missing} / 실제키={list(df.columns)}')
    for src, dst in PDF_JSON_COLS.items():
        if src not in df.columns:
            df[src] = None
    df = df[list(PDF_JSON_COLS.keys())].rename(columns=PDF_JSON_COLS)
    return df


def _probe_pdf_has_data(session: rq.Session, isin: str, day_str: str) -> bool:
    """
...
def find_latest_pdf_biz_day(session: rq.Session, isin: str, max_lookback: int = BIZDAY_LOOKBACK) -> str:
    """
...
def resolve_trd_dates(session: rq.Session, sample_isin: str) -> list:
    """
...
def load_pdf_from_db(
    ...
    sql = text(f"""
        SELECT
            `ETF코드`, `ETF명`, `수집일자`, `티커`, `구성종목명`,
            `계약수`, `금액`, `시가총액`, `{PDF_WEIGHT_COL}` AS 비중
        FROM `{PDF_TABLE}`
        WHERE {' AND '.join(where)}
        ORDER BY `ETF코드`, `수집일자`, `{PDF_WEIGHT_COL}` DESC
    """)
...
def pdf_etf_display_name(g: pd.DataFrame, code: str) -> str:
...
# 스냅샷
# ─────────────────────────────────────────────────────────────
def pdf_resolve_snapshot_dates(all_dates: list) -> dict[str, pd.Timestamp | None]:
    dates = sorted(pd.to_datetime(all_dates).unique())
    if not dates:
        return {name: None for name, _ in PDF_SNAPSHOT_DEFS}
    out: dict[str, pd.Timestamp | None] = {}
    latest_idx = len(dates) - 1
    for name, offset in PDF_SNAPSHOT_DEFS:
        idx = latest_idx - int(offset)
        out[name] = dates[idx] if idx >= 0 else None
    return out


def pdf_snapshot_meta_html(snaps: dict[str, pd.Timestamp | None]) -> str:
    bits = []
    for name, _ in PDF_SNAPSHOT_DEFS:
        d = snaps.get(name)
        if d is None:
...
def pdf_build_snapshot_wide(
    etf_df: pd.DataFrame,
    snaps: dict[str, pd.Timestamp | None],
    top_n: int | None = PDF_TOP_N,
) -> pd.DataFrame:
    """
    ...
    for cur, prev in PDF_SNAPSHOT_CHAIN:
    ...
    if top_n is not None:
        ...
        chg_cols = [
            f"vs최근_{n}" for n, _ in PDF_SNAPSHOT_DEFS if n != "DB최근"
        ] + [f"chainΔ_{cur}" for cur, _ in PDF_SNAPSHOT_CHAIN]
        for col in chg_cols:
        ...
        exited = wide[wide["_miss_DB최근"] & wide[[n for n, _ in PDF_SNAPSHOT_DEFS if n != "DB최근"]].notna().any(axis=1)]
...
# 그라데이션 / 셀 렌더
# ─────────────────────────────────────────────────────────────
def pdf_delta_bg(delta: float | None, max_pp: float = PDF_GRAD_MAX_PP) -> str:
...
def pdf_fmt_weight_cell(weight, delta, missing: bool = False) -> tuple[str, str]:
    """
    ...
    if missing or weight is None or (isinstance(weight, float) and np.isnan(weight)):
        ...
        if delta is None or (isinstance(delta, float) and np.isnan(delta)):
            return f"0.000{tag}", pdf_delta_bg(None) if not missing else pdf_delta_bg(-1e-9)
        d = float(delta)
        sign = "+" if d >= 0 else ""
        cls = "up" if d > 0 else ("down" if d < 0 else "flat")
        inner = f'0.000 <span class="dlt {cls}">({sign}{d:.3f})</span>{tag}'
        return inner, pdf_delta_bg(d)
...
def pdf_render_etf_weight_table(wide: pd.DataFrame, snaps: dict) -> str:
    if wide.empty:
    ...
    chain_delta_for = {cur: f"chainΔ_{cur}" for cur, _prev in PDF_SNAPSHOT_CHAIN}
    chain_legend = ", ".join(f"{cur}←{prev}" for cur, prev in PDF_SNAPSHOT_CHAIN) or "(없음)"
    ...
    for i, r in wide.iterrows():
        ...
        for name, _ in PDF_SNAPSHOT_DEFS:
            ...
            inner, style = pdf_fmt_weight_cell(w, delta, missing=(miss and name == "DB최근"))
...
# 전체 요약 순위
# ─────────────────────────────────────────────────────────────
def pdf_build_universe_snapshot_frame(df: pd.DataFrame, snaps: dict) -> pd.DataFrame:
    ...
    for code in ordered + leftovers:
        ...
        wide = pdf_build_snapshot_wide(g, etf_snaps, top_n=None)
        if wide.empty:
            continue
        wide = wide.copy()
        wide.insert(0, "ETF코드", str(code))
        wide.insert(1, "ETF명", pdf_etf_display_name(g, str(code)))
        parts.append(wide)
    if not parts:
        return pd.DataFrame()
...
def pdf_rank_table_html(
    frame: pd.DataFrame,
    value_col: str,
    title: str,
    top_n: int = PDF_SUMMARY_TOP_N,
    as_delta: bool = False,
...
def pdf_render_summary_section(universe: pd.DataFrame) -> str:
    if universe.empty:
    ...
    cards = [
        pdf_rank_table_html(universe, "DB최근", f"비중 상위 (DB최근) TOP{PDF_SUMMARY_TOP_N}", as_delta=False),
        pdf_rank_table_html(
            universe_ex,
            "DB최근",
            f"비중 상위 · 삼성전자·하이닉스 제외 TOP{PDF_SUMMARY_TOP_N}",
            as_delta=False,
        ),
    ]
    for col, label in (
        ...
        cards.append(
            pdf_rank_table_html(
                universe, col, f"{label} · 비중확대 TOP{PDF_SUMMARY_TOP_N}", as_delta=True, direction="up"
            )
        )
        cards.append(
            pdf_rank_table_html(
                universe, col, f"{label} · 비중축소 TOP{PDF_SUMMARY_TOP_N}", as_delta=True, direction="down"
            )
        )

    # chainΔ: 연속 시점 구간별 (직전 스냅샷 대비)
    for i, (cur, prev) in enumerate(PDF_SNAPSHOT_CHAIN):
        col = f"chainΔ_{cur}"
...
# HTML 대시보드
# ─────────────────────────────────────────────────────────────
def pdf_etf_snaps_for_group(g: pd.DataFrame, snaps: dict) -> dict:
    etf_dates = set(g["수집일자"].tolist())
    etf_snaps = dict(snaps)
    etf_snaps["DB최근"] = g["수집일자"].max()
    for name, _ in PDF_SNAPSHOT_DEFS:
...
def pdf_build_dashboard_html(df: pd.DataFrame) -> str:
    if df.empty:
        return "<html><body><h1>데이터 없음</h1></body></html>"

    all_dates = sorted(df["수집일자"].unique())
    snaps = pdf_resolve_snapshot_dates(all_dates)
    latest = snaps.get("DB최근")
    ...
    universe = pdf_build_universe_snapshot_frame(df, snaps)
    summary_html = pdf_render_summary_section(universe)
    ...
    for group, codes in ISU_CODES.items():
        ...
        for code in group_codes:
            ...
            if g.empty:
                continue
            seen.add(str(code))
            title = pdf_etf_display_name(g, str(code))
            etf_snaps = pdf_etf_snaps_for_group(g, snaps)
            wide = pdf_build_snapshot_wide(g, etf_snaps, PDF_TOP_N)
...
<section class="etf-block" id="etf-{code}">
  ...
  <p class="meta">{pdf_snapshot_meta_html(etf_snaps)}</p>
  <h3>스냅샷 비중 (연속 시점 대비 변화)</h3>
  {pdf_render_etf_weight_table(wide, etf_snaps)}
</section>
""")
...
<body>
  <header>
    ...
    <p class="sub">{pdf_snapshot_meta_html(snaps)}</p>
...
# 실행
# ─────────────────────────────────────────────────────────────
def collect_etf_pdf(session=None) -> bool:
    """ETF PDF 수집. session이 있으면 재사용(로그인·close는 호출측)."""
    own = session is None
    if own:
        session = rq.Session()
        if not krx_login(session):
            return False
    try:
        return _collect_etf_pdf_body(session)
    finally:
        if own:
            try:
                session.close()
...
def _collect_etf_pdf_body(session) -> bool:
    ...
    if frames:
        ...
        out_file = os.path.join(PDF_RESULTS_DIR, f"krx_pdf_result_{date_tag}.csv")
...
def generate_weight_dashboard(
    ...
    if start_date is None:
        start_date = PDF_DASH_START_DATE
    if end_date is None:
        end_date = PDF_DASH_END_DATE
    if etf_codes is None:
        etf_codes = PDF_DASH_ETF_FILTER if PDF_DASH_ETF_FILTER is not None else isu_codes_ordered()
    ...
    html_out = pdf_build_dashboard_html(df)
    os.makedirs(PDF_RESULTS_DIR, exist_ok=True)
    latest = df["수집일자"].max().strftime("%Y%m%d")
    out_path = os.path.join(PDF_RESULTS_DIR, f"etf_pdf_weight_dashboard_{latest}.html")
...
def pdf_main(session=None):
    """PDF 수집 + 비중 대시보드. RUN_COLLECT_PDF / RUN_DASH_PDF 를 따름."""
    print("ETF PDF — 수집 + 비중 대시보드")
    print(f"· RUN_COLLECT_PDF={RUN_COLLECT_PDF} / RUN_DASH_PDF={RUN_DASH_PDF}")
    if RUN_COLLECT_PDF:
        print("\n" + "=" * 50)
        print("PDF 수집")
        print("=" * 50)
        ok = collect_etf_pdf(session=session)
        if not ok:
            print("⚠️ 수집 단계에서 문제가 있었습니다. (대시보드는 계속 시도)")
    else:
        print("· PDF 수집 스킵 (RUN_COLLECT_PDF=False)")
    if RUN_DASH_PDF:
        generate_weight_dashboard()
    else:
        print("· PDF 대시보드 스킵 (RUN_DASH_PDF=False)")


def main():
    """①시세 ②PDF ③모멘텀대시보드 ④구성비중대시보드 — 단계별 try/except."""
    print("=" * 60)
    print("ETF v7.0 — 시세·PDF 수집 + 대시보드")
    print(
        f"· RUN_COLLECT_OHLCV={RUN_COLLECT_OHLCV} / RUN_COLLECT_PDF={RUN_COLLECT_PDF} / "
        f"RUN_DASH_MOMENTUM={RUN_DASH_MOMENTUM} / RUN_DASH_PDF={RUN_DASH_PDF}"
    )
    print("=" * 60)

    shared_session = None
    need_krx = RUN_COLLECT_OHLCV or RUN_COLLECT_PDF
    if need_krx:
        shared_session = rq.Session()
        shared_session.headers.update(KRX_ETF_OHLCV_HEADERS)
        try:
            if not krx_login(shared_session):
                print("⚠️ KRX 로그인 실패 — 수집 단계는 개별적으로 재시도하거나 스킵될 수 있습니다.")
                try:
            ...
            print(f"⚠️ KRX 로그인 예외: {e}")
            print(traceback.format_exc())
            shared_session = None

    try:
        # ① ETF 시세 수집
        if RUN_COLLECT_OHLCV:
            print("\n" + "=" * 50)
            print("① ETF OHLCV 데이터 수집 (KRX MDCSTAT04301)")
            print("=" * 50)
            try:
                result = collect_etf_ohlcv_data(max_etf=None, session=shared_session)
                print(f"\n수집 결과: {result}")
                ...
                print(f"⚠️ ① ETF OHLCV 수집 실패: {e}")
                print(traceback.format_exc())
        else:
            print("\n· ① ETF OHLCV 수집 스킵 (RUN_COLLECT_OHLCV=False)")

        # ② ETF PDF 수집
        if RUN_COLLECT_PDF:
            print("\n" + "=" * 50)
            print("② ETF PDF 수집 (MDCSTAT05001)")
            print("=" * 50)
            try:
                ok = collect_etf_pdf(session=shared_session)
                if not ok:
                    print("⚠️ PDF 수집이 정상 완료되지 않았습니다.")
            except Exception as e:
                print(f"⚠️ ② ETF PDF 수집 실패: {e}")
                print(traceback.format_exc())
        else:
            print("\n· ② ETF PDF 수집 스킵 (RUN_COLLECT_PDF=False)")
    finally:
    ...
    # ③ 모멘텀·수익률 대시보드
    if RUN_DASH_MOMENTUM:
        print("\n" + "=" * 50)
        print("③ ETF 모멘텀·수익률 대시보드")
        print("=" * 50)
        try:
            momentum_df = analyze_etf_momentum(
                visualize=True, save_path="etf_momentum_rankings.png"
            )
            if momentum_df is not None and not momentum_df.empty:
                print(f"\n✓ 모멘텀 분석 완료: {len(momentum_df)}개 ETF 분석됨")
        except Exception as e:
            print(f"⚠️ ③ 모멘텀 대시보드 실패: {e}")
            print(traceback.format_exc())
        try:
            html_out = save_etf_code_name_returns_html()
            if html_out:
                print(f"\n✓ ETF_CODE_NAME HTML 저장: {html_out}")
        except Exception as e:
            print(f"⚠️ ETF_CODE_NAME HTML 저장 실패: {e}")
            print(traceback.format_exc())
    else:
        print("\n· ③ 모멘텀 대시보드 스킵 (RUN_DASH_MOMENTUM=False)")

    # ④ 구성비중 대시보드
    if RUN_DASH_PDF:
        print("\n" + "=" * 50)
        print("④ ETF PDF 구성비중 대시보드")
        print("=" * 50)
        try:
            generate_weight_dashboard()
        except Exception as e:
            print(f"⚠️ ④ PDF 비중 대시보드 실패: {e}")
            print(traceback.format_exc())
    else:
        print("\n· ④ PDF 비중 대시보드 스킵 (RUN_DASH_PDF=False)")
